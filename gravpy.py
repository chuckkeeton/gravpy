...
class Gravpy(object):
    def __init__(self, carargs, polargs, modelargs, show_plot=True, include_caustics=True,
                 image=np.random.uniform(0, 1, 2), recurse_depth=3):
    ...
    def mapping(self, v):
        """Function used for root finding. \'v\' is the true position in the image plane
        (what we want to find). Not intended for vectorized inputs. Returns the deflection
        vector along with the inverse-magnification matrix (aka the Jacobian).
        """
    ...
    def subdivide_cells(self, cells, cell_depth):
        """Divides each cell in a list of cells (given by [[p1, p2],[p3, p4]]) into four smaller cells.
        On the cartesian grid, the points are ordered as so:
        p2--------p4         |
        |         |      II  |  I
        |         |     -----|-----
        |         |      III |  IV
        ...
        # but still retaining one of its vertices. This happens four times, shrinking each time
        # towards one of the four vertices, leaving four quarter cells that together make up the original cell.
    ...
    def for_points5_wrapper_cached(self, cells, mag_cells, cell_depth):
        """Function that subdivdes given cells, computes the magnification values for new points,
        merges the magnification values from \'mag_cells\' with the newly computed magnifcation values,
        and returns the magnification values and cells where a critical curve was detected.
        """
        ...
        q1, q2, q3, q4 = [np.delete(q, i, axis=1) for q, i in
                          zip([q1, q2, q3, q4], [3, 1, 0, 2])]  # remove the cells that stay the same
        ...
        # combine old values with newly calculated values
        mag_combined = np.vstack([np.insert(m, i, c, axis=1) for m, i, c in zip([m1, m2, m3, m4], [3, 1, 0, 2],
                                                                                [c4, c2, c1,
                                                                                 c3])])

        mag_change_mask = self.cell_mag_change(mag_combined)  # which cells had a change in magnification

        # return mag values of selected cells (used for the next level in gridding)
        # and the cells themselves that had a mag change
    ...
    def points5(self, xran, yran, caustics_mode=False):
        ...
        Algorithm works by vectorizing each level of cell-size, handling each level in one
        complete calculation before proceeding to the next.
        """
