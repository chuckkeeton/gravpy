...
def buildrelations(xran, yran, coord, modelargs):
    ...
    xx, yy = np.meshgrid(xran, yran,
                         sparse=True)  # sparse =True for sparse matrices, gives a row (x) and column (y) vector.
...
def mapping(v, u, modelargs):
    """Function used for root finding. u is image (non-variable),
    and v is the true position in the image plane (variable).
    Not intended for vectorized inputs. Returns the deflection vector along
    with the inverse-magnification matrix (aka the Jacobian).
    """
...
def potdefmag(x, y, modelargs):
    """The wrapper used to find the phi values given models' parameters.
    The output is (6,x) where x is the length of the x,y arguments given in the invocation.
    This command seeks out the correct module to contact for each model calculation.
    """
    phi2Darray = []
    ...
    for mass_component in modelargs:
        ...
        # add up contributions from different parameters (from same model) into one result phiarray.
        # Equivalent to a final sum over all different models simliar to the one after this for-loop,
        # but this sum makes appending easier (due to dimensionality issues - each model will/(most likely)
        # have different number of mass components).
        phi2Darray.append(np.sum(new_arr,
                                 axis=2))
...
def cond_break(x, y, modelargs, conds, function_calls):
    """Experimental function that decides whether to vectorize multiple mass components
    or just iterate over a for loop. Current break is at 10 mass components
    """
    num = x.shape[1]
...
def cond_break_vec(x, y, modelargs, conds, function_calls):
    """This code is essentially some filtering and ordering to seperate cases basedon conditions
    and then recombine resulting phiarrays back into the same order they came in argument-wise.
    For example, in the 'sie' module, there is a split in calculations for elliptical and spherical cases.
    This wrapper will split the incoming arguments into their respective function calls and
    merge the outputs back together in the order they came in.
    """
...
def mag_of_cells(cells, modelargs):
    """Takes a list of cells and returns the magnification values for each point in the cells.
    Retains shape and order of the original list of cells.
    """
...
def cell_mag_change(cells_mag):
    """Takes a list of magnification values of a list of cells and returns a boolean mask for
    which the magnification changes across a cell. Uses numpy vectorization.
    """
...
def subdivide_cells(cells, grid_spacing, cell_depth):
    """Divides each cell in a list of cells (given by [[p1, p2],[p3, p4]])
    into four smaller cells. On the cartesian grid, the points are ordered as so:
    p2--------p4         |
    |         |      II  |  I
    |         |     -----|-----
    |         |      III |  IV
    ...
    # below code uses broadcasting to 'shrink' each cell into a fourth of its size,
    # but still retaining one of its vertices. This happens four times,
    # shrinking each time towards one of the four vertices,
    # leaving four quarter cells that together make up the original cell.
...
def points5_wrapper(cells, modelargs):
    """Takes a list of cells and returns the cells for which the magnification changes sign.
    Function itself is a condensed command for three lines of code,
    which I did not want to write over and over again.
    """
...
def points5(xran, yran, spacing, modelargs, recurse_depth=3, caustics_mode=False):
    """A vectorized approach to bulding a 'recursive' subgrid without recursion.
    Algorithm works by vectorizing each level of cell-size, handling each level in
    one complete calculation before proceeding to the next.
    """
    x = xran[0:-1]
    y = yran[0:-1]
    ...
    gridm_x_y = np.vstack(
        (np.dstack((xs, xs, xs + spacing, xs + spacing)), np.dstack((ys, ys + spacing, ys, ys + spacing))))
...
def generate_ranges(carargs, polargs, modelargs, caustics=True):
    ...
    If caustics is set to False, then [critx, crity] and [causticsx, causticsy]
    are not returned-- Instead just [x,y] and [r,theta] are returned.
    """
...
def transformations(car_ranges, pol_ranges, spacing, modelargs, recurse_depth=3):
    """Generates the subgridding points (more points around the critical curve),
    the transformations from image plane to source plane, and the Delaunay Triangulization object for plotting.
    """
    x, y = car_ranges
    ...
    transformed = np.array(
        carmapping(stack[:, 0], stack[:, 1], modelargs))  # transform pairs from image to source plane
...
def run(carargs, polargs, modelargs, show_plot=True, caustics=True, image=np.random.uniform(-1, 1, 2), recurse_depth=3):
    """The master command that wraps and executes all the commands to run a gridding example.
    Use this function (excusively) when using this module.
    """
    ...
    critx, crity = args[2] or None
    causticsx, causticsy = args[3] or None

    stack, transformed, dpoints = transformations((x, y), polargrids, spacing, bettermodelargs,
