...
pmodelargs2 = [alpha(1,0,0,0.,0,0.,1.5)]
...
if callgraph:
    with PyCallGraph(output=GraphvizOutput(output_file=filepath)):
        example = gravlens(pcarargs,ppolargs,pmodelargs, image=pimage, show_plot=False, include_caustics=False)
        example.run()
else:
    example = gravlens(pcarargs,ppolargs2,pmodelargs2)
    example.run()
